...
ncpus = snakemake.resources['cpus']

out_png = snakemake.output['png']
out_csv = snakemake.output['csv']

def calculate_metrics(atac_peaks, RP_count, RN_count, scores):
...
f = partial(calculate_metrics, atac_peaks, RP_count, RN_count)

with Pool(ncpus) as pool:
    res = pool.map(f, range(0,int(atac_peaks['score'].quantile(.99)), 1), chunksize = 1e2)

ratio_count_df_pivot = pd.concat(list(res))
