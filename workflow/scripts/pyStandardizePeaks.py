...
def merge_peaks_by_chr(standard_peak):
    standard_peak = standard_peak.sort_values('-log10(pvalue)', ascending = False)
    i=0
...
def parse_single_sample(file, chrom_sizes_df, out_dir = '.'):
    ...
    standard_peak = standard_peak.groupby('chr').apply(merge_peaks_by_chr)
